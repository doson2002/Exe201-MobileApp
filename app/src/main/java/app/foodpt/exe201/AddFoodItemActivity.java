...
public class AddFoodItemActivity extends AppCompatActivity {
    ...
    private EditText etInventoryQuantity, etPrice ;
    Button saveButton;
    Uri imageUri;
    String imageUrl;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linearAddFoodType = findViewById(R.id.linearAddFoodType);
         ...
         etInventoryQuantity = findViewById(R.id.etInventoryQuantity);
        ...
        // Sự kiện cho statusSpinner
        statusSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String selectedStatus = parent.getItemAtPosition(position).toString();
                if ("Có Sẵn".equals(selectedStatus)) {
                    etInventoryQuantity.setVisibility(View.VISIBLE);
                } else {
                    etInventoryQuantity.setVisibility(View.GONE);
                    etInventoryQuantity.setText("0"); // Mặc định giá là 0
                }
            }
...
// Sự kiện cho categorySpinner
        categorySpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String selectedCategory = parent.getItemAtPosition(position).toString();
                if ("Trả phí".equals(selectedCategory)) {
    ...
    private void sendDataToApi(String imgUrl) {
        ...
        String quantityStr = etInventoryQuantity.getText().toString().trim();
        String priceStr = etPrice.getText().toString().trim();
        String status = statusSpinner.getSelectedItem().toString();
        String category = categorySpinner.getSelectedItem().toString();


        // Kiểm tra các trường dữ liệu xem có hợp lệ không
        if (!validateFields(foodName, description, quantityStr, priceStr, imgUrl, supplierId)) {
    ...
    private boolean validateFields(String foodName, String description, String quantityStr, String priceStr, String imgUrl, int supplierId) {
        // Kiểm tra tên món ăn
        if (foodName.isEmpty()) {
            ((EditText) findViewById(R.id.etFoodName)).setError("Tên món ăn không được để trống");
            return false;
        }

        // Kiểm tra mô tả
        if (description.isEmpty()) {
            ((EditText) findViewById(R.id.etDescription)).setError("Mô tả không được để trống");
            return false;
        }

        // Kiểm tra số lượng
        if (quantityStr.isEmpty()) {
            etInventoryQuantity.setError("Số lượng không được để trống");
            return false;
        }
        ...
        if (imgUrl == null || imgUrl.isEmpty()) {
            Toast.makeText(this, "Vui lòng tải lên hình ảnh món ăn", Toast.LENGTH_SHORT).show();
            return false;
        }

        // Kiểm tra supplierId
        if (supplierId == -1) {
            Toast.makeText(this, "Lỗi xác định Supplier ID", Toast.LENGTH_SHORT).show();
            return false;
        }

        // Kiểm tra giá trị số cho số lượng
        try {
            Integer.parseInt(quantityStr);
        } catch (NumberFormatException e) {
            etInventoryQuantity.setError("Số lượng phải là một số hợp lệ");
            return false;
        }

        // Kiểm tra giá trị số cho giá
        try {
            Double.parseDouble(priceStr);
        } catch (NumberFormatException e) {
            etPrice.setError("Giá phải là một số hợp lệ");
            return false;
        }
